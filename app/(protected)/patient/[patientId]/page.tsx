          ...
          <Card className="bg-white rounded-xl p-6 w-full lg:w-[70%] border-none space-y-6">
            <div className="flex flex-col md:flex-row md:flex-wrap md:items-center xl:justify-between gap-y-4 md:gap-x-0">
              ...
              <SmallCard
                ...
                value={data?.date_of_birth ? format(new Date(data.date_of_birth), "dd-MM-aaaa") : "N/A"}

              />
              <SmallCard label={"Número de teléfono"} value={data?.phone!} />
            </div>

            <div className="flex flex-col md:flex-row md:flex-wrap md:items-center xl:justify-between gap-y-4 md:gap-x-0">
              <SmallCard label="Estado civil" value={data?.marital_status!} />
              <SmallCard label="Grupo sanguíneo" value={data?.blood_group!} />
              <SmallCard label="Dirección" value={data?.address!} />
            </div>
            ...
            <div className="flex flex-col md:flex-row md:flex-wrap md:items-center xl:justify-between gap-y-4 md:gap-x-0">
              ...
              <SmallCard
                ...
                value={
                  data?.lastVisit
                    ? format(data?.lastVisit!, "dd-MM-aaaa")
                    : "Sin última visita"
                }
              />
            </div>
        ...
        <div className="bg-white p-4 rounded-md mb-8">
          ...
          <div className="mt-4 flex gap-4 flex-wrap text-xs text-gray-500">
            ...
            <Link className="p-3 rounded-md bg-rose-100" href={`#`}>
              Prueba de laboratorio y resultado
            </Link>
            {patientId === "self" && (
              <Link
