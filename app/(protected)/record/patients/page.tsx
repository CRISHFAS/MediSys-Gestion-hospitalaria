...
const PatientList = async (props: SearchParamsProps) => {
  ...
  const renderRow = (item: PatientProps) => {
    ...
    const lastTreatment =
      item?.appointments[0]?.medical[0]?.treatment_plan || null;
    ...
    return (
      ...
      >
        ...
        <td className="hidden xl:table-cell">
          {lastVisit ? (
            format(lastVisit?.created_at, "dd-MM-aaaa HH:mm:ss")
          ) : (
            <span className="text-gray-500 text-sm italic">Sin última visita</span>
          )}
        </td>
        <td className="hidden xl:table-cell">
          {lastTreatment ? (
