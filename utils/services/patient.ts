...
export async function getPatientById(id: string) {
  try {
    ...
    if (!patient) {
      return { success: false, error: true, msg: "Paciente no encontrado" };
    }
    return { success: true, error: false, data: patient };
  } catch (error) {
    console.error(error);
    return { success: false, error: true, msg: "Error al obtener los datos del paciente" };
  }
}
