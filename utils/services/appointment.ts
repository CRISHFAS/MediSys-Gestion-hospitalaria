...
const buildQuery = (
  ...
  // Date filtering conditions if date exists
  const dateConditions: Prisma.AppointmentWhereInput = date
    ? {
        appointment_date: {
          gte: new Date(date),
  ...
  // Status filtering conditions if status exists
  const statusConditions: Prisma.AppointmentWhereInput = status
    ? {
        status: status as any,
      }
    : {};

  // Combine all conditions with AND if any exist
  const combinedQuery: Prisma.AppointmentWhereInput =
    id || search || date || status
      ? {
          AND: [
            ...
            ...(Object.keys(dateConditions).length > 0 ? [dateConditions] : []),
            ...(Object.keys(statusConditions).length > 0
              ? [statusConditions]
              : []),
          ],
        }
  ...
  try {
    ...
    const [data, totalRecord] = await Promise.all([
      db.appointment.findMany({
        where: buildQuery(id, search, date, status),
        skip: SKIP,
        take: LIMIT,
        select: {
          id: true,
